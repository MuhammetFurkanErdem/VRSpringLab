...
public class SpringForceVector : MonoBehaviour
{
    ...
    private void Update()
    {
        ...
        float force =
            springSimulation.SpringForceNewtons;
        ...
        float length =
            Mathf.Min(
                Mathf.Abs(force) * visualScale,
                maxLength
            );

        // Normal durumda yay kuvveti yukarı.
        // Displacement negatif olursa yön fiziksel olarak tersine döner.
        Vector3 direction =
            force >= 0f
                ? Vector3.up
                : Vector3.down;
